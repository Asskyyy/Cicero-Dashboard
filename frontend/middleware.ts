...
export default auth(async (req) => {
  ...
  if (req.nextUrl.pathname.startsWith('/teacher') && req.auth?.user.role !== 'TEACHER') {
    return NextResponse.rewrite(new URL('/_error', req.url));
  }
  if (req.nextUrl.pathname.startsWith('/student') && req.auth?.user.status !== 'ACTIVE') {
    return NextResponse.rewrite(new URL('/_error', req.url));
  }
  if (req.nextUrl.pathname.startsWith('/home') && req.auth?.user.status !== 'ACTIVE') {
...
export const config = {
  matcher: ['/((?!.+\\.[\\w]+$|_next).*)', '/', '/(api|trpc)(.*)', '/admin', '/teacher'],
};
