...
import { getAllLessons, getAllClassrooms } from '@backend/services/academy';
import Edt from './btn/edt';
import Del from './btn/del';
import Link from 'next/link';
import Pagination from '../pagination/pagination';
import { PageProps } from '@/types/pagination';
import { fetchAssignment } from '@backend/services/assignments';
        ...
        {data.map((assignment) => (
          <tr key={assignment.id}>
            <td className="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
              <h5 className="font-medium text-black dark:text-white">{assignment.lesson.name}</h5>
            </td>
            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
              <h5 className="text-sm  text-black dark:text-white">{assignment.task}</h5>
            </td>
            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
              <p className="text-sm  text-black dark:text-white">{assignment.teacher.name}</p>
            </td>
            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
              <p className="text-sm  text-black dark:text-white">{assignment.classroom.name}</p>
            </td>
            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
              {assignment.fileUrl ? (
            ...
            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
              <p className="text-sm  text-black dark:text-white">{assignment.time}</p>
            </td>
            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
              <div className="flex items-center space-x-3.5">
                <Del assignment={assignment} />
                <Edt lessons={lessons} classrooms={classrooms} assignment={assignment} />
              </div>
            </td>
          </tr>
        ))}
