...
import { getAssignmentbyTeacherId, getLessonbyTeacherId } from '@backend/services/teacher';
import { getAllClassrooms } from '@backend/services/academy';
import { AssignmentResult } from '@/lib/teacher';
import Edt from './btn/edt';
import Del from './btn/del';
const TbodyAssignment = async () => {
  const user = await currentUser();
  const result = await getAssignmentbyTeacherId(user?.id);
  const [classrooms] = await Promise.all([getAllClassrooms()]);
  const { data } = await getLessonbyTeacherId(user?.id);
          ...
          assingmentItem.lesson.map((lesson) => (
            <tr key={`${lesson.id}-${lesson.name}`}>
              <td className="border-b border-[#eee] px-4 py-5 pl-9 dark:border-strokedark xl:pl-11">
                <ul className="font-medium text-black dark:text-white py-5">{lesson.name}</ul>
              </td>
              <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                <ul className="text-sm text-black dark:text-white py-5">
                  {lesson.assingment.map((assignment) => (
                    <li key={assignment.id} className="text-sm text-black dark:text-white py-1.5">
                      {assignment.task}
                    </li>
                  ))}
                </ul>
              ...
              <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                <ul className="text-sm text-black dark:text-white py-5">
                  {lesson.assingment.map((assignment) => (
                    <li key={assignment.id} className="text-sm text-black dark:text-white py-1">
                  ...
                  {lesson.assingment.map((assignment) => (
                    <li key={assignment.id} className="text-sm text-black dark:text-white py-1">
                      <div className="flex items-center space-x-3.5">
                        <Edt lessons={data} classrooms={classrooms} assignment={assignment} />
